      ...
      <div className="bg-red-50 p-5 pl-10 rounded-[3rem] border border-red-200">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          <p className="text-sm text-urbik-rose/70 font-medium max-w-md">
            Si eliminas tu cuenta, se perderán todas tus publicaciones activas.
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 z-100 flex items-center justify-center px-4">
            ...
            >
              ...
              <button
                onClick={() => setShowDeleteModal(false)}
                className="absolute cursor-pointer top-8 right-8 text-urbik-muted hover:text-urbik-black"
              >
                <X size={28} />
              </button>
              <h3 className="text-2xl text-center font-display font-bold mb-4 text-urbik-black">
                ¿Absolutamente seguro?
              </h3>
              <div className="bg-red-50 p-5 rounded-2xl border border-red-100 mb-8 text-sm text-urbik-rose font-medium">
                Escribe{" "}
                <span className="text-urbik-rose font-black">
                  &quot;{DELETE_KEYWORD}&quot;
                </span>{" "}
                para confirmar la eliminación de{" "}
                <span className="font-bold underline">{itemName}</span>.
              </div>
              <input
                type="text"
                value={confirmText}
