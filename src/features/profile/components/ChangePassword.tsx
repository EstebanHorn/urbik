...
const SecuritySection = () => {
  ...
  const inputBaseClasses = "w-full px-6 py-4 rounded-full bg-white border border-gray-300 focus:ring-2 focus:ring-black outline-none transition-all font-medium text-black";
      ...
      <div className="bg-white p-8 rounded-[2rem] border border-gray-200 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Cambia tu contraseña periódicamente para protegerte</p>
          </div>
        </div>
        <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-gray-50 w-full max-w-lg rounded-[3rem] p-8 md:p-12 shadow-2xl relative animate-in fade-in zoom-in duration-300">
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              ...
              {message.text && (
                <p className={`text-center text-sm font-bold ${message.type === 'error' ? 'text-red-500' : 'text-green-600'}`}>
                  {message.text}
                </p>
              )}
