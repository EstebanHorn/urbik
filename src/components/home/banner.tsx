...
export default function Banner({ items }: BannerProps) {
  ...
  const operationParam =
    operation === "alquilar" ? "RENT" : operation === "comprar" ? "SALE" : null;

  const handleSelect = (suggestion: SearchSuggestion) => {
    if (
      suggestion.type === "PROPERTY_SEARCH" &&
      suggestion.parsedFilters &&
      operationParam &&
      !suggestion.parsedFilters.operationType
    ) {
      onSelectSuggestion({
        ...suggestion,
        parsedFilters: {
          ...suggestion.parsedFilters,
          operationType: operationParam,
        },
      });
    } else {
  ...
  return (
    <>
      ...
      <div
        className={`relative w-full h-[230px] md:h-[650px] -mb-25 md:overflow-hidden bg-geora-black transition-all duration-1000 ease-out ${
        ...
        <div className="absolute inset-0 z-20 flex flex-col md:flex-row items-center justify-center px-4 sm:px-6 md:px-16 lg:px-54 py-8 md:py-10">
          ...
          >
            <h1 className="mb-3 text-2xl font-black tracking-tighter text-white sm:text-5xl md:text-6xl lg:text-7xl drop-shadow-2xl">
              Tu próximo hogar
              <br className="hidden md:block" /> empieza acá
              ...
              {suggestions.length > 0 && (
                <ul className="absolute z-9999 w-full mt-2 bg-white border border-gray-100 shadow-2xl rounded-2xl overflow-hidden">
                  {suggestions.map((suggestion, index) => {
                    ...
                    const agencyCity =
                      suggestion.type === "REALESTATE_USER" &&
                    ...
                    return (
                      <li
                        ...
                        className="px-4 py-3 cursor-pointer hover:bg-gray-50 transition-colors flex justify-between items-center text-sm border-b last:border-none z-50 border-gray-50"
                      ...
                      >
                        ...
                        <span
                          className={`shrink-0 ml-2 text-[10px] uppercase tracking-wider font-bold px-2 py-1 rounded-full ${badge.className}`}
                        >
                          {badge.label}
                        </span>
                      </li>
