...
export default function Versus({
    ...
    try {
      const propertiesData = selectedProperties.map((p, index) => {
        const nearbyProperty = selectedProperties.find((other, otherIdx) => {
          ...
          return (
            Math.abs(p.latitude - other.latitude) < 0.005 &&
            Math.abs(p.longitude - other.longitude) < 0.005
          );
        });
        return {
          title: p.title,
  ...
  }, [selectedProperties.length, isVersusMode, getAiAnalysis]);
  ...
  const stats = [
    ...
    {
      label: "Superficie",
      key: "area" as keyof Property,
      unit: " m²",
      lowerBetter: false,
    },
    {
      label: "Habitaciones",
      key: "rooms" as keyof Property,
      unit: "",
      lowerBetter: false,
    },
    {
      label: "Baños",
      key: "bathrooms" as keyof Property,
      unit: "",
      lowerBetter: false,
    },
  ];
            ...
            {loadingAi ? (
              ...
              >
                <video
                  ...
                  muted
                  playsInline
                  className="w-20 h-20 object-cover brightness-125"
                />
              </motion.div>
            ) : aiAnalysis ? (
              <motion.div
              ...
              >
                <div className="bg-urbik-emerald/5 border border-urbik-emerald/20 p-6 rounded-2xl backdrop-blur-sm">
                  ...
                  <p className="text-lg font-medium italic leading-relaxed text-emerald-50/90">
                    {aiAnalysis}
                  </p>
                </div>
              </motion.div>
                    ...
                    {selectedProperties.map((p) => {
                      ...
                      return (
                        <td key={p.id} className="p-6 text-center">
                          <span
                            className={`text-xl md:text-2xl font-black tracking-tighter transition-all duration-500 ${getStatColor(val, stat.key, stat.lowerBetter)}`}
                          >
                            {stat.unit === "$" && "$"}
                            {val?.toLocaleString() || "-"}
                            {stat.unit !== "$" && stat.unit}
