...
export default function SmartDescription({ description, context }: SmartDescriptionProps) {
  ...
  const showLoadingState = loading || (isActive && (!description || description.length < 20));
      ...
      <div className="w-full relative z-10 p-2">
        <div className="w-full flex items-center justify-between mb-1">
          <div className="flex items-center gap-3">
            ...
            />
            <span className={`text-xs font-black italic uppercase transition-colors ${isActive ? 'text-white' : 'text-white/30'}`}>
              Urbik <span className={isActive ? 'text-urbik-emerald' : 'text-white/30'}>Smart Description</span>
            </span>
          </div>
          
          <div className="flex items-center gap-3">
            <span className={`hidden sm:block text-[8px] font-bold uppercase tracking-widest transition-colors ${isActive ? 'text-urbik-emerald' : 'text-white/20'}`}>
        ...
        <AnimatePresence mode="wait">
          {isActive && (
            ...
            >
              <div className="p-4 rounded-xl min-h-[80px] flex items-center justify-center">
                {showLoadingState ? (
                  ...
                  >
                    <div className="bg-urbik-emerald/5 border border-urbik-emerald/10 p-4 rounded-xl backdrop-blur-sm relative">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[9px] font-bold text-urbik-emerald/80 uppercase tracking-widest">Sugerencia de IA</span>
                      </div>
