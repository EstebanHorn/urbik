...
const completion = await groq.chat.completions.create({
  messages: [
    {
      role: "system",
      content: "Eres un asistente técnico que describe ubicaciones de forma objetiva. No interpretas datos, solo los relatas de forma elegante y breve."
    },
    {
      role: "user",
      content: prompt,
  ...
  temperature: 0.1, 
    ...
    const responseText = completion.choices[0]?.message?.content || "No se pudo generar un análisis.";

    return NextResponse.json({ text: responseText });

  } catch (error: any) {
    console.error("Error en Groq API:", error.message || error);
