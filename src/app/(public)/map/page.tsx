...
import type { MapBounds, MapProperty, BaseLayerId } from "@/components/map/types";
...
const PropertiesSidebar = dynamicImport(
  () => import("@/components/map/PropertiesSidebar").then((mod) => mod.PropertiesSidebar),
  {
    ssr: false,
    loading: () => <div className="p-6 text-sm text-slate-400">Cargando propiedades...</div>,
  }
);
...
export default function MapPage() {
  ...
  const lat = searchParams.get("lat") ? parseFloat(searchParams.get("lat") as string) : defaultLat;
  const lon = searchParams.get("lon") ? parseFloat(searchParams.get("lon") as string) : defaultLon;
  ...
  const syncFiltersToUrl = useCallback(
    (nextFilters: FilterState) => {
      const nextQuery = applyFiltersToParams(new URLSearchParams(searchParams.toString()), nextFilters).toString();
 ...
 const fetchFilteredProperties = useCallback(async (visibleBounds: MapBounds, currentFilters: FilterState) => {
    ...
    const signature = JSON.stringify({ bounds: visibleBounds, filters: currentFilters });
    ...
    try {
      ...
      const res = await fetch(`/api/properties/in-bounds?${query.toString()}`, { signal: controller.signal });
      if (!res.ok) { setProperties([]); return; }
      ...
      const normalizedList = rawList.map((p: RawProperty) => ({
        ...
        inMapBounds: 
          p.lat >= visibleBounds.minLat && p.lat <= visibleBounds.maxLat &&
          p.lon >= visibleBounds.minLon && p.lon <= visibleBounds.maxLon
      }));
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => fetchFilteredProperties(boundsRef.current as MapBounds, filters), 220);
        ...
        >
          <div className="shrink-0 p-4 shadow-sm md:hidden">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-slate-800">Resultados ({properties.length})</h2>
              <button onClick={() => setShowMobileList(false)} className="cursor-pointer p-2 text-slate-500">
                <X size={20} />
              </button>
            </div>
          </div>
          ...
          <div className="pointer-events-auto absolute bottom-6 left-1/2 z-50 w-max -translate-x-1/2 md:hidden">
            ...
            >
              {showMobileList ? (
                ...
                <><List className="h-4 w-4" /> Ver Lista <span className="rounded-full bg-slate-700 px-2 text-xs">{properties.length}</span></>
              )}
            </button>
          </div>
        </main>
